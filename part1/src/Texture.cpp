...
// copy constructor
Texture::Texture(const Texture& oldTex) {
	m_pixelData = oldTex.m_pixelData;
}
...
void Texture::CopyTextureToGPU(bool texIsDirty) {
	LoadTexture(m_filepath, texIsDirty);
}
...
void Texture::LoadTexture(const std::string filepath, bool texIsDirty){
	// Set member variable
  m_filepath = filepath;
